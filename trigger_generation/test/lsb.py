...
def main():
    ...
    metricdir = "./test_result/LSB/"+class_name
    ...
    watermark = np.uint8([1,1,1,0,0,1,0,0]).reshape(1,8)
    watermark = np.tile(watermark,4)
    watermark = watermark.repeat(32, axis=0)
    watermark = watermark[:,:,np.newaxis]
    watermark = watermark.repeat(3, axis=2)
    ...
    with torch.no_grad():
        ...
        # for i in range (imgNum):
            ...
            cover_img_np  =np.array(cover_img)
            tunc_conver = (cover_img_np - cover_img_np % 2)
            stego_img_np = tunc_conver + watermark
            ...
            resultImg_paper = torch.cat([ cover_B,  stego_img, (stego_img-cover_B), 10*(stego_img-cover_B)], 0)
