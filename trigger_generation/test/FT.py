...
def main():
    ...
    with torch.no_grad():
        ...
        # for i in range (imgNum):
            ...
            stego_img = poison_frequency(cover_img).float()
...
def poison_frequency(x_train):
    ...
    x_train = np.transpose(x_train, (0, 2, 3, 1))  #转成B, H, W, C
    x_train *= 255.
    x_train = RGB2YUV(x_train)

    # transfer to frequency domain
    x_train = DCT(x_train, 32)  # (idx, ch, w, h)

    # plug trigger frequency
    for i in range(x_train.shape[0]):
        for ch in [1, 2]:
            for w in range(0, x_train.shape[2], 32):
                for h in range(0, x_train.shape[3], 32):
                    for pos in [(31, 31), (15, 15)]:
                        x_train[i][ch][w + pos[0]][h + pos[1]] += 30


    x_train = IDCT(x_train, 32)  # (idx, w, h, ch)

    x_train = YUV2RGB(x_train)

    x_train = np.transpose(x_train, (0, 3, 1, 2))  #转成B, C, H, W,
